...
#[repr(i32)]
pub enum RawCommand {
    About                    = ae_sys::PF_Cmd_ABOUT as _,
    GlobalSetup              = ae_sys::PF_Cmd_GLOBAL_SETUP as _,
    GlobalSetdown            = ae_sys::PF_Cmd_GLOBAL_SETDOWN as _,
    ParamsSetup              = ae_sys::PF_Cmd_PARAMS_SETUP as _,
    SequenceSetup            = ae_sys::PF_Cmd_SEQUENCE_SETUP as _,
    SequenceResetup          = ae_sys::PF_Cmd_SEQUENCE_RESETUP as _,
    SequenceFlatten          = ae_sys::PF_Cmd_SEQUENCE_FLATTEN as _,
    SequenceSetdown          = ae_sys::PF_Cmd_SEQUENCE_SETDOWN as _,
    DoDialog                 = ae_sys::PF_Cmd_DO_DIALOG as _,
    FrameSetup               = ae_sys::PF_Cmd_FRAME_SETUP as _,
    Render                   = ae_sys::PF_Cmd_RENDER as _,
    FrameSetdown             = ae_sys::PF_Cmd_FRAME_SETDOWN as _,
    UserChangedParam         = ae_sys::PF_Cmd_USER_CHANGED_PARAM as _,
    UpdateParamsUi           = ae_sys::PF_Cmd_UPDATE_PARAMS_UI as _,
    Event                    = ae_sys::PF_Cmd_EVENT as _,
    GetExternalDependencies  = ae_sys::PF_Cmd_GET_EXTERNAL_DEPENDENCIES as _,
    CompletelyGeneral        = ae_sys::PF_Cmd_COMPLETELY_GENERAL as _,
    QueryDynamicFlags        = ae_sys::PF_Cmd_QUERY_DYNAMIC_FLAGS as _,
    AudioRender              = ae_sys::PF_Cmd_AUDIO_RENDER as _,
    AudioSetup               = ae_sys::PF_Cmd_AUDIO_SETUP as _,
    AudioSetdown             = ae_sys::PF_Cmd_AUDIO_SETDOWN as _,
    ArbitraryCallback        = ae_sys::PF_Cmd_ARBITRARY_CALLBACK as _,
    SmartPreRender           = ae_sys::PF_Cmd_SMART_PRE_RENDER as _,
    SmartRender              = ae_sys::PF_Cmd_SMART_RENDER as _,
    GetFlattenedSequenceData = ae_sys::PF_Cmd_GET_FLATTENED_SEQUENCE_DATA as _,
    TranslateParamsToPrefs   = ae_sys::PF_Cmd_TRANSLATE_PARAMS_TO_PREFS as _,
    SmartRenderGpu           = ae_sys::PF_Cmd_SMART_RENDER_GPU as _,
    GpuDeviceSetup           = ae_sys::PF_Cmd_GPU_DEVICE_SETUP as _,
    GpuDeviceSetdown         = ae_sys::PF_Cmd_GPU_DEVICE_SETDOWN as _,
}

impl From<RawCommand> for ae_sys::PF_Cmd {
    fn from(v: RawCommand) -> Self {
        v as ae_sys::PF_Cmd
    }
}

// AE 2026+ has been observed sending alternate selector values for some commands.
impl RawCommand {
    fn normalize(v: ae_sys::PF_Cmd) -> i32 {
        match v as i32 {
            7425 => ae_sys::PF_Cmd_QUERY_DYNAMIC_FLAGS as i32,
            7431 => ae_sys::PF_Cmd_SMART_PRE_RENDER as i32,
            other => other,
        }
    }
}

impl From<ae_sys::PF_Cmd> for RawCommand {
    fn from(v: ae_sys::PF_Cmd) -> Self {
        let v = RawCommand::normalize(v);
        match v {
            x if x == ae_sys::PF_Cmd_ABOUT as i32 => Self::About,
            x if x == ae_sys::PF_Cmd_GLOBAL_SETUP as i32 => Self::GlobalSetup,
            x if x == ae_sys::PF_Cmd_GLOBAL_SETDOWN as i32 => Self::GlobalSetdown,
            x if x == ae_sys::PF_Cmd_PARAMS_SETUP as i32 => Self::ParamsSetup,
            x if x == ae_sys::PF_Cmd_SEQUENCE_SETUP as i32 => Self::SequenceSetup,
            x if x == ae_sys::PF_Cmd_SEQUENCE_RESETUP as i32 => Self::SequenceResetup,
            x if x == ae_sys::PF_Cmd_SEQUENCE_FLATTEN as i32 => Self::SequenceFlatten,
            x if x == ae_sys::PF_Cmd_SEQUENCE_SETDOWN as i32 => Self::SequenceSetdown,
            x if x == ae_sys::PF_Cmd_DO_DIALOG as i32 => Self::DoDialog,
            x if x == ae_sys::PF_Cmd_FRAME_SETUP as i32 => Self::FrameSetup,
            x if x == ae_sys::PF_Cmd_RENDER as i32 => Self::Render,
            x if x == ae_sys::PF_Cmd_FRAME_SETDOWN as i32 => Self::FrameSetdown,
            x if x == ae_sys::PF_Cmd_USER_CHANGED_PARAM as i32 => Self::UserChangedParam,
            x if x == ae_sys::PF_Cmd_UPDATE_PARAMS_UI as i32 => Self::UpdateParamsUi,
            x if x == ae_sys::PF_Cmd_EVENT as i32 => Self::Event,
            x if x == ae_sys::PF_Cmd_GET_EXTERNAL_DEPENDENCIES as i32 => {
                Self::GetExternalDependencies
            }
            x if x == ae_sys::PF_Cmd_COMPLETELY_GENERAL as i32 => Self::CompletelyGeneral,
            x if x == ae_sys::PF_Cmd_QUERY_DYNAMIC_FLAGS as i32 => Self::QueryDynamicFlags,
            x if x == ae_sys::PF_Cmd_AUDIO_RENDER as i32 => Self::AudioRender,
            x if x == ae_sys::PF_Cmd_AUDIO_SETUP as i32 => Self::AudioSetup,
            x if x == ae_sys::PF_Cmd_AUDIO_SETDOWN as i32 => Self::AudioSetdown,
            x if x == ae_sys::PF_Cmd_ARBITRARY_CALLBACK as i32 => Self::ArbitraryCallback,
            x if x == ae_sys::PF_Cmd_SMART_PRE_RENDER as i32 => Self::SmartPreRender,
            x if x == ae_sys::PF_Cmd_SMART_RENDER as i32 => Self::SmartRender,
            x if x == ae_sys::PF_Cmd_GET_FLATTENED_SEQUENCE_DATA as i32 => {
                Self::GetFlattenedSequenceData
            }
            x if x == ae_sys::PF_Cmd_TRANSLATE_PARAMS_TO_PREFS as i32 => {
                Self::TranslateParamsToPrefs
            }
            x if x == ae_sys::PF_Cmd_SMART_RENDER_GPU as i32 => Self::SmartRenderGpu,
            x if x == ae_sys::PF_Cmd_GPU_DEVICE_SETUP as i32 => Self::GpuDeviceSetup,
            x if x == ae_sys::PF_Cmd_GPU_DEVICE_SETDOWN as i32 => Self::GpuDeviceSetdown,
            other => {
                log::warn!("Unknown PF_Cmd value {other}, treating as About");
                Self::About
            }
        }
    }
