...
#[repr(i32)]
pub enum Error {
    Generic                  = ae_sys::A_Err_GENERIC as _,
    Struct                   = ae_sys::A_Err_STRUCT as _,
    Parameter                = ae_sys::A_Err_PARAMETER as _,
    OutOfMemory              = ae_sys::A_Err_ALLOC as _,
    WrongThread              = ae_sys::A_Err_WRONG_THREAD as _,
    ConstProjectModification = ae_sys::A_Err_CONST_PROJECT_MODIFICATION as _,
    MissingSuite             = ae_sys::A_Err_MISSING_SUITE as _,
    InternalStructDamaged    = ae_sys::PF_Err_INTERNAL_STRUCT_DAMAGED as _,
    InvalidIndex             = ae_sys::PF_Err_INVALID_INDEX as _,
    UnrecogizedParameterType = ae_sys::PF_Err_UNRECOGNIZED_PARAM_TYPE as _,
    InvalidCallback          = ae_sys::PF_Err_INVALID_CALLBACK as _,
    BadCallbackParameter     = ae_sys::PF_Err_BAD_CALLBACK_PARAM as _,
    InterruptCancel          = ae_sys::PF_Interrupt_CANCEL as _,
    CannotParseKeyframeText  = ae_sys::PF_Err_CANNOT_PARSE_KEYFRAME_TEXT as _,
    Reserved11               = ae_sys::A_Err_RESERVED_11 as _,
    StringNotFound           = ae_sys::suiteError_StringNotFound as _,
    StringBufferTooSmall     = ae_sys::suiteError_StringBufferTooSmall as _,
    InvalidParms             = ae_sys::suiteError_InvalidParms as _,
    Unknown10007             = UNKNOWN_ERR_10007 as _,
    NotInComputeCache        = ae_sys::A_Err_NOT_IN_CACHE_OR_COMPUTE_PENDING as _,
    None                     = ae_sys::PF_Err_NONE as _,
}

impl From<ae_sys::PF_Err> for Error {
    fn from(v: ae_sys::PF_Err) -> Self {
        match v as i32 {
            x if x == ae_sys::A_Err_GENERIC as i32 => Self::Generic,
            x if x == ae_sys::A_Err_STRUCT as i32 => Self::Struct,
            x if x == ae_sys::A_Err_PARAMETER as i32 => Self::Parameter,
            x if x == ae_sys::A_Err_ALLOC as i32 => Self::OutOfMemory,
            x if x == ae_sys::A_Err_WRONG_THREAD as i32 => Self::WrongThread,
            x if x == ae_sys::A_Err_CONST_PROJECT_MODIFICATION as i32 => {
                Self::ConstProjectModification
            }
            x if x == ae_sys::A_Err_MISSING_SUITE as i32 => Self::MissingSuite,
            x if x == ae_sys::PF_Err_INTERNAL_STRUCT_DAMAGED as i32 => {
                Self::InternalStructDamaged
            }
            x if x == ae_sys::PF_Err_INVALID_INDEX as i32 => Self::InvalidIndex,
            x if x == ae_sys::PF_Err_UNRECOGNIZED_PARAM_TYPE as i32 => {
                Self::UnrecogizedParameterType
            }
            x if x == ae_sys::PF_Err_INVALID_CALLBACK as i32 => Self::InvalidCallback,
            x if x == ae_sys::PF_Err_BAD_CALLBACK_PARAM as i32 => Self::BadCallbackParameter,
            x if x == ae_sys::PF_Interrupt_CANCEL as i32 => Self::InterruptCancel,
            x if x == ae_sys::PF_Err_CANNOT_PARSE_KEYFRAME_TEXT as i32 => {
                Self::CannotParseKeyframeText
            }
            x if x == ae_sys::A_Err_RESERVED_11 as i32 => Self::Reserved11,
            x if x == ae_sys::suiteError_StringNotFound as i32 => Self::StringNotFound,
            x if x == ae_sys::suiteError_StringBufferTooSmall as i32 => {
                Self::StringBufferTooSmall
            }
            x if x == ae_sys::suiteError_InvalidParms as i32 => Self::InvalidParms,
            x if x == UNKNOWN_ERR_10007 as i32 => Self::Unknown10007,
            x if x == ae_sys::A_Err_NOT_IN_CACHE_OR_COMPUTE_PENDING as i32 => {
                Self::NotInComputeCache
            }
            x if x == ae_sys::PF_Err_NONE as i32 => Self::None,
            other => {
                log::warn!("Unknown PF_Err value {other}, treating as Generic");
                Self::Generic
            }
        }
    }
...
impl From<Error> for ae_sys::PF_Err {
