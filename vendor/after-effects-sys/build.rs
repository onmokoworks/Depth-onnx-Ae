...
fn main() {
    ...
    if let Ok(mut content) = std::fs::read_to_string(out_path.join("bindings.rs")) {
        ...
        if content.contains("pub __bindgen_anon_1") && content.contains("PF_Point__bindgen_ty_1") {
        ...
        // AE 25.x renamed PF_ParamDef::name_do_not_use_directly -> name
        if content.contains("pub name : [A_char ; 32usize] ,")
            && !content.contains("name_do_not_use_directly")
        {
            content = content.replace(
